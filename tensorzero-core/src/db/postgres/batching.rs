...
use super::inference_queries::{
    build_insert_chat_inference_io_query, build_insert_chat_inferences_query,
    build_insert_json_inference_io_query, build_insert_json_inferences_query,
};
use super::model_inferences::{
    build_insert_model_inference_io_query, build_insert_model_inferences_query,
};
                ...
                process_channel_with_capacity_and_timeout(
                    ...
                    move |buffer| {
                        ...
                        async move {
                            ...
                            match build_insert_chat_inference_io_query(&buffer) {
                                Ok(mut qb) => {
                                    if let Err(e) = qb.build().execute(&pool).await {
                                        tracing::error!(
                ...
                process_channel_with_capacity_and_timeout(
                    ...
                    move |buffer| {
                        ...
                        async move {
                            ...
                            match build_insert_json_inference_io_query(&buffer) {
                                Ok(mut qb) => {
                                    if let Err(e) = qb.build().execute(&pool).await {
                                        tracing::error!(
                ...
                process_channel_with_capacity_and_timeout(
                    ...
                    move |buffer| {
                        ...
                        async move {
                            ...
                            match build_insert_model_inference_io_query(&buffer) {
                                Ok(mut qb) => {
                                    if let Err(e) = qb.build().execute(&pool).await {
                                        tracing::error!(
