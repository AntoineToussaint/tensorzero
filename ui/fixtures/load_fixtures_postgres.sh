...
# - "chat_inference_examples.jsonl" -> chat_inferences + chat_inference_io (Done)
# - "json_inference_examples.jsonl" -> json_inferences + json_inference_io (Done)
# - "model_inference_examples.jsonl" -> model_inferences + model_inference_io (Done)
...
TRUNCATE TABLE tensorzero.chat_inference_io CASCADE;
TRUNCATE TABLE tensorzero.json_inference_io CASCADE;
TRUNCATE TABLE tensorzero.model_inference_io CASCADE;
...
INSERT INTO tensorzero.chat_inferences (
    ...
    processing_time_ms, ttft_ms, tags, tool_choice,
...
load_jsonl "small-fixtures/chat_inference_examples.jsonl" "tensorzero.chat_inference_io" "
INSERT INTO tensorzero.chat_inference_io (
...
# ClickHouse stores these as String (JSON-encoded), so we use ->> to extract text then cast to jsonb
# NULLIF handles empty strings that would fail jsonb cast
load_jsonl "small-fixtures/json_inference_examples.jsonl" "tensorzero.json_inference_io" "
INSERT INTO tensorzero.json_inference_io (
...
# ClickHouse stores these as String (JSON-encoded), so we use ->> to extract text then cast to jsonb
# NULLIF handles empty strings that would fail jsonb cast
load_jsonl "small-fixtures/model_inference_examples.jsonl" "tensorzero.model_inference_io" "
INSERT INTO tensorzero.model_inference_io (
...
UNION ALL
SELECT 'chat_inference_io', count(*) FROM tensorzero.chat_inference_io
UNION ALL
SELECT 'json_inferences', count(*) FROM tensorzero.json_inferences
UNION ALL
SELECT 'json_inference_io', count(*) FROM tensorzero.json_inference_io
UNION ALL
SELECT 'model_inferences', count(*) FROM tensorzero.model_inferences
UNION ALL
SELECT 'model_inference_io', count(*) FROM tensorzero.model_inference_io
UNION ALL
