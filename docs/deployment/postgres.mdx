...
<Step title="Enable TensorZero optimizers (optional)">

Some TensorZero optimizers (e.g. [DICL](/optimization/dicl)) store data in Postgres and require the [`pgvector`](https://github.com/pgvector/pgvector) extension.

If you plan to use these optimizers, you must install `pgvector` in your Postgres instance.
...
When applying migrations (next step), pass the `--enable-optimizer-postgres-migrations` flag to apply the additional optimizer schema:
...
```bash
docker compose run --rm gateway --run-postgres-migrations --enable-optimizer-postgres-migrations
```
...
docker run --rm --network host \
    ...
    tensorzero/gateway --run-postgres-migrations --enable-optimizer-postgres-migrations
```
