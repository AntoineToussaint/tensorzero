...
By default, cache data is stored in ClickHouse.
If Valkey is available (i.e. `TENSORZERO_VALKEY_URL` is set), the gateway automatically uses Valkey for inference caching instead.
